                ...
                <div class="col-12 col-md-6 order-md-2 order-first">
                    <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <a href="{{ route('pedoman-pelayanan.create') }}" class="btn bg-gradient-primary btn-sm mb-0">+&nbsp; Tambah Item</a>
                        </ol>
                        <!-- pop up item start-->
                            ...
                            <div class="d-flex justify-content-center  flex-column">
                                <div class="table-responsive">
                                    <table class="table " id="table1">
                                        ...
                                        <tbody>
                                            @foreach ($reglist as $key => $reglist)
                                            <tr>
                                                <th>{{ $key +1 }}
                                                </th>
                                                <th>{{ $reglist->name }}<span class="badge bg-info">{{ $reglist->kd }}</span></th>
                                                <th>
                                                    <ul>
                                                        @foreach ($itemdoc as $key => $itemdoc)
                                                        <li>
                                                            {{ $itemdoc->tgl_tetap }},
                                                        </li>
                                                        @endforeach
                                                ...
                                                <th>
                                                    <div>
                                                        <a class="btn btn-transtaparant" href="{{ route('pedoman-pelayanan.edit', $reglist->id) }}"><i class="bi bi-pencil"></i></a>

                                                    </div>
                                                    <div>
                                                        {!! Form::open([
                                                        'method' => 'DELETE',
                                                        'route' => ['pedoman-pelayanan.destroy', $reglist->id],
